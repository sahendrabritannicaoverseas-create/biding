...
import { Microscope, TrendingUp, CheckCircle, DollarSign, Search, ArrowRight, FileText } from 'lucide-react';
...
export function LabDashboard() {
  ...
  const statCards = [
    { title: 'Testing Opportunities', value: stats.availableOpportunities, icon: FileText, color: 'bg-yellow-500', link: '/rfps' },
    { title: 'Active Bids', value: stats.activeBids, icon: TrendingUp, color: 'bg-green-500', link: '/bids' },
    { title: 'Active Projects', value: stats.activeContracts, icon: CheckCircle, color: 'bg-blue-500', link: '/projects' },
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div><h1 className="text-3xl font-bold text-gray-900">Welcome back, {profile?.full_name}</h1><p className="text-gray-600 mt-1">Lab testing opportunities and project management</p></div>
        <Link href="/rfps" className="inline-flex items-center space-x-2 bg-yellow-600 text-white px-6 py-3 rounded-lg hover:bg-yellow-700 transition font-semibold"><Search className="w-5 h-5" /><span>Browse Opportunities</span></Link>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        <div className="p-6 border-b border-gray-200"><div className="flex justify-between items-center"><h2 className="text-xl font-bold text-gray-900">Lab Testing Opportunities</h2><Link href="/rfps" className="text-sm font-medium text-blue-600 hover:text-blue-700 flex items-center">View all<ArrowRight className="w-4 h-4 ml-1" /></Link></div></div>
        <div className="divide-y divide-gray-200">
          {recentRFPs.length === 0 ? (
            <div className="p-12 text-center"><Microscope className="w-12 h-12 text-gray-400 mx-auto mb-4" /><p className="text-gray-600">No lab testing opportunities available at the moment.</p></div>
          ) : recentRFPs.map(rfp => (
