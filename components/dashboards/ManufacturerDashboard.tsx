...
import { Factory, TrendingUp, CheckCircle, DollarSign, Search, ArrowRight, FileText } from 'lucide-react';
...
export function ManufacturerDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      const { data: rfpsData } = await supabase.from('rfps').select('*').eq('status', 'active').in('requirement_category', ['manufacturing', 'contract_manufacturing', 'api_supply']).order('created_at', { ascending: false }).limit(5);
  ...
  const statCards = [
    { title: 'Available Opportunities', value: stats.availableOpportunities, icon: FileText, color: 'bg-orange-500', link: '/rfps' },
    ...
    { title: 'Total Revenue', value: `$${stats.totalRevenue.toLocaleString()}`, icon: DollarSign, color: 'bg-slate-500', link: '/payments' },
  ];
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div><h1 className="text-3xl font-bold text-gray-900">Welcome back, {profile?.full_name}</h1><p className="text-gray-600 mt-1">Explore manufacturing opportunities and manage your contracts</p></div>
        <Link href="/rfps" className="inline-flex items-center space-x-2 bg-orange-600 text-white px-6 py-3 rounded-lg hover:bg-orange-700 transition font-semibold"><Search className="w-5 h-5" /><span>Browse Opportunities</span></Link>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        <div className="p-6 border-b border-gray-200"><div className="flex justify-between items-center"><h2 className="text-xl font-bold text-gray-900">New Manufacturing Opportunities</h2><Link href="/rfps" className="text-sm font-medium text-blue-600 hover:text-blue-700 flex items-center">View all<ArrowRight className="w-4 h-4 ml-1" /></Link></div></div>
        <div className="divide-y divide-gray-200">
          {recentRFPs.length === 0 ? (
            <div className="p-12 text-center"><Factory className="w-12 h-12 text-gray-400 mx-auto mb-4" /><p className="text-gray-600">No manufacturing opportunities available at the moment.</p></div>
          ) : recentRFPs.map(rfp => (
            <Link key={rfp.id} href={`/rfps/${rfp.id}`} className="p-6 hover:bg-gray-50 transition block">
              <div className="flex justify-between items-start">
                <div className="flex-1"><div className="flex items-center space-x-3"><h3 className="font-semibold text-gray-900">{rfp.title}</h3><span className="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-700">Open</span></div><p className="text-sm text-gray-600 mt-1">{rfp.study_type} • {rfp.target_geography?.join(', ')}</p><div className="flex items-center space-x-4 mt-3 text-sm text-gray-500"><span>RFP #{rfp.rfp_number}</span><span>•</span><span>{rfp.bids_count || 0} bids</span><span>•</span><span>Deadline: {new Date(rfp.submission_deadline).toLocaleDateString()}</span></div></div>
